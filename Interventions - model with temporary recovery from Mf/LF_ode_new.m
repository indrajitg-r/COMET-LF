function ydot = LF_ode_new(t,y,param)
parameters;
theta_vh = param(1);
lambda_E=param(2);
omega_E = param(3);
...
lambda_ht = lambda_W/52;
%% model equations
dotS_h= pi_h - beta1*theta_vh*I_v*(S_h/N_h) - mu_h*S_h +i1*R_h;
dotE_h= beta1*theta_vh*I_v*(S_h/N_h) - (omega_E + lambda_E + mu_h)*E_h;
dotW_h= lambda_E*E_h +(1-p1)*p2*impulsive_MDA(t)*M_h - (p1*impulsive_MDA(t) + omega_W + alpha1 +  lambda_ht + mu_h)*W_h;
dotM_h= lambda_ht*W_h - (p2*impulsive_MDA(t) + alpha2 + mu_h)*M_h;
dotC_h= alpha1*W_h + alpha2*M_h - mu_h*C_h;
% dotR_ht = (1-p1)*p2*impulsive_MDA(t)*M_h - (alpha2 + omega_W + lambda_ht + mu_h)*R_ht;
dotR_h= omega_E*E_h + p1*impulsive_MDA(t)*W_h + omega_W*W_h + p1*p2*impulsive_MDA(t)*M_h - (i1+mu_h)*R_h;
dotS_v= pi_v - beta1*theta_hv*S_v*(M_h/N_h) - mu_v*S_v;
dotE_v=beta1*theta_hv*S_v*(M_h/N_h) - (lambda_v + mu_v)*E_v;
dotI_v=lambda_v*E_v - mu_v*I_v;
